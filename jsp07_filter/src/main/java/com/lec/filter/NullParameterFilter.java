...
/*
	...
	jsp/servlet에서 들어온 요청을 처리하기 전 올바른 요청인지 or 특정 자원에 접근권한이 있는지 등을
	사전에 처리할 수 있도록 한다. 또한 응답하는 데이터를 변경하거나 응답 취소할 수 있다.
	즉, Http의 request와 response의 내용을 변경할 수 있도록 지원하는 객체다.
	
	필터는 객체형태로 클라이언트에서 요구하는 요청과 웹서버(jsp,servlet...)의 사이에 위치하여
	클라이언트의 요청을 제어할 수 있도록 한다. 또한, 웹서버와 응답처리 사이에 위치하여 응답 제어도 가능하다.
	
	즉, 필터는 웹브라우저의 요청을 필터의 체인으로 연결해서 다음단계로 보낼 수도 있고
	다른 형태의 결과를 웹브라우저에 전송할 수도 있다.
	
	A. 필터 구현에 필요한 3개의 핵심 class
	...
	B. Filter인터페이스의 관련메서드
		1. init()		: 필터를 초기화할 때 사용
		2. doFilter()	: 필터기능을 수행하는 추상메서드, FilterChain.chain()메서드로 다음 필터로 이동
		3. destory()	: 필터가 웹컨테이너에서 삭제될 때 호출되는 메서드
		
	C. 필터의 구현절차
		1. init()메서드 호출해서 초기화 작업 수행
		2. doFilter()
			a. request파라미터를 이용해서 요청의 필터작업을 수행
			b. chain.doFilter(req, res)를 이용하여 체인의 다음필터로 이동
			c. response를 이용해 응답의 필터링 작업을 수행
