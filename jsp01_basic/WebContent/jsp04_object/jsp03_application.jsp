...
<!-- 
	3. application 기본객체
	웹애플리케이션 전역에 걸쳐 사용되는 정보를 저장할 수 있다.
	예를 들어 application객체를 사용해 초기설정정보, 서버정보, 웹애플리케이션에서 제공하는 정보를
	공통으로 공유해서 사용할 수 있다.
	
	서블릿규약에선 웹애플리케이션 전체에 걸쳐 사용(공유)할 수 있는 초기화 파라키터를 정의하고 있는데
	초기화 파라미터는 web.xml의 <context-param>태그를 사용해 추가할 수 있다.
	
	[ web.xml ]
	web.xml파일은 웹애플리케이션을 위한 설정정보를 가지고 있는 파일이다. jsp프로그래밍을 할 때
	꼭 필요하진 않지만 환경설정파일로서 중요한 파일이다. 반드시 WEB-INF폴더에 위치해야 한다.
	다른 폴더에 위치할 경우 인식하지 못한다.
	
	[ application객체의 초기화 메서드 ]
	a. getInitParameter(String name) : name이라는 키로 초기화 파라미터 값을 읽을 수 있다.
	...
	d. log() : application객체는 웹컨테이너가 사용하는 로그파일에 로그메세지를 기록할 수 있는 메서드
			   ...톰캣설치폴더/logs폴더에 localhost_log.yyyy-mm-dd.log파일에 메세지가 기록된다
 -->
<html>
<head>
<meta charset="UTF-8">
