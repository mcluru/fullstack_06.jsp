...
<!-- 
	...
	3. out(javax.servlet.jsp.JspWriter) : jsp페이지의 생성결과를 출력할 때 사용하는 출력문자 스트림
 ...
 <!-- 
 	...
 	1) out객체의 출력메서드
 		a. print()   : 데이터를 출력(줄바꿈x)
 		b. println() : 데이터를 출력(줄바꿈o)
 		c. newLine() : 줄바꿈
 		d. write() 	 : dom태그를 html로 출력
 		
 	2) out객체의 buffer관련 메서드
 		a. getBufferSize() : 버퍼크기 리턴
 		b. getRemaining()  : 버퍼의 잔량 리턴
 		c. clear() 		   : 버퍼전체를 비움, 이미 flush되어있다면 exception발생
 		d. clearBuffer()   : 버퍼전체를 비움, 이미 flush되어있다면 exception발생x
 		e. flush() 		   : 버퍼를 flush처리
 		f. isAutoFlush()   : 자동flush여부를 boolean으로 리턴
